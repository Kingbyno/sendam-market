...
export async function confirmReceipt(transactionId: string, userId: string) {
  try {
    const transaction = await prisma.purchase.update({
      where: {
        id: transactionId,
        buyerId: userId, // Only buyer can confirm receipt
      },
      data: {
        status: "CONFIRMED",
        confirmedAt: new Date(),
    ...
    // TODO: Trigger auto-release after confirmation
    // await scheduleAutoRelease(transaction)
